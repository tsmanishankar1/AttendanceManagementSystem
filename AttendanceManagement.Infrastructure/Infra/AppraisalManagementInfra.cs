...
namespace AttendanceManagement.Infrastructure.Infra
{
    public class AppraisalManagementInfra : IAppraisalManagementInfra
    {
        ...
        */
        ...
        {
            if (fileName.Contains("..") || Path.GetFileName(fileName) != fileName) throw new ArgumentException("Invalid file name");
            return Path.Combine(_workspace, fileName);
        }
                        ...
                        if (employeePerformanceReviews.Count > 0)
                        {
                            ...
                            var alreadyUploaded = await _context.EmployeePerformanceReviews
                                .Where(x =>
                                    ...
                                    (uploadMisSheetRequest.Quarter == null || x.Quarter == uploadMisSheetRequest.Quarter) && (uploadMisSheetRequest.Month == null || x.Month == uploadMisSheetRequest.Month))
                                .Select(x => x.EmpId)
                                .Distinct()
                                .ToListAsync();
                            ...
                            {
                                throw new ConflictException($"The following staff have already been uploaded " + string.Join(", ", alreadyUploaded));
                            }
                        }
                        else
                        {
        ...
        public async Task<string> MoveToAgmApproval(AgmApprovalTab agmApprovalRequest)
        {
            ...
            if (agmApprovalRequest.SelectedRows.Count() == 0) throw new MessageNotFoundException("No rows selected");
            ...
            var existingApprovalReviewIds = await _context.AgmApprovals
                 .Where(x =>
                     ...
                     (agmApprovalRequest.Quarter == null || x.Quarter == agmApprovalRequest.Quarter) && (agmApprovalRequest.Month == null || x.Month == agmApprovalRequest.Month))
        ...
        public async Task<string> GenerateAppraisalLetter(int createdBy, int year)
        {
            ...
            {
                throw new MessageNotFoundException("Please upload the employee appraisal sheet before generating the salary revision letter");
            }
        ...
        {
            var staffId = await _context.StaffCreations.FirstOrDefaultAsync(s => s.Id == letterAcceptance.AcceptedBy && s.IsActive == true);
            if (staffId == null) throw new MessageNotFoundException("Staff not found");
            string prefixWithoutDC = $"Appraisal_Letter_Without_DC_{staffId.StaffId}_";
            string prefixWithDC = $"Appraisal_Letter_With_DC_{staffId.StaffId}_";
            var letterGeneration = await _context.LetterGenerations
                .Where(x => (x.FileName.StartsWith(prefixWithoutDC) || x.FileName.StartsWith(prefixWithDC)) && x.IsActive)
                .OrderByDescending(x => x.CreatedUtc)
                .FirstOrDefaultAsync();
            ...
            var latestAcceptance = await _context.EmployeeAcceptances
                .Where(x => x.FileId == letterGeneration.Id && x.EmpId == empId)
        ...
        public async Task<string> MoveSelectedStaff(SelectedEmployeesRequest selectedEmployeesRequest)
        {
            ...
            var alreadySelected = await _context.SelectedNonProductionEmployees
                .Where(x => employeeIds.Contains(x.EmployeeId) && x.AppraisalId == selectedEmployeesRequest.AppraisalId && !x.IsActive && x.IsCompleted == true
                            && (x.AppraisalId != 1 || (x.Year == selectedEmployeesRequest.Year
                            && (selectedEmployeesRequest.Quarter == null || x.Quarter == selectedEmployeesRequest.Quarter) && (selectedEmployeesRequest.Month == null || x.Month == selectedEmployeesRequest.Month))))
                .Select(x => x.EmployeeId)
                .ToListAsync();
        ...
        public async Task<List<SelectedEmployeesResponseSelectedRows>> GetSelectedNonProductionEmployees(int appraisalId, int year, string? quarter, int? month)
        {
            var selectedEmployees = await (from staff in _context.SelectedNonProductionEmployees
                                           join s in _context.StaffCreations on staff.EmployeeId equals s.StaffId
                                           where staff.IsCompleted == false && staff.AppraisalId == appraisalId && staff.Year == year && (quarter == null || staff.Quarter == quarter) && (month == null || staff.Month == month)
                                           select new SelectedEmployeesResponseSelectedRows
                                           {
                                               EmpId = staff.EmployeeId,
                                               EmpName = staff.EmployeeName,
        ...
        public async Task<string> CreateKra(KraDto kraDto)
        {
            ...
            foreach (var item in selectedRows)
            {
                ...
                foreach (var staffId in item.StaffId)
                {
                    ...
                    {
                        StaffId = staffId,
                        Message = "You have been assigned a new KRA",
                        IsActive = true,
                        CreatedBy = kraDto.CreatedBy,
                        CreatedUtc = DateTime.UtcNow
                    };
        ...
        public async Task<List<KraResponse>> GetKra(int createdBy, int appraisalId, int year, string? quarter, int? month)
        {
            ...
            var getKraRaw = await (
                ...
                where kra.Year == year
                      && (quarter == null || kra.Quarter == quarter)
                      && (month == null || kra.Month == month)
        ...
        public async Task<string> CreateSelfEvaluation(SelfEvaluationRequest selfEvaluationRequest)
        {
            ...
            var staffId = await _context.StaffCreations.FirstOrDefaultAsync(s => s.Id == selfEvaluationRequest.CreatedBy && s.IsActive == true);
            var staffFullName = $"{staffId.FirstName}{(string.IsNullOrWhiteSpace(staffId.LastName) ? "" : " " + staffId.LastName)}";
            string submittedDateTime = selfEvaluation.CreatedUtc.ToLocalTime().ToString("dd-MMM-yyyy 'at' HH:mm:ss");
            ...
            var managerName = $"{approver1.FirstName}{(string.IsNullOrWhiteSpace(approver1.LastName) ? "" : " " + approver1.LastName)}";
            var requestedBy = await _context.StaffCreations
                .Where(s => s.Id == selfEvaluationRequest.CreatedBy && s.IsActive == true)
                .Select(s => $"{s.FirstName}{(string.IsNullOrWhiteSpace(s.LastName) ? "" : " " + s.LastName)}")
                .FirstOrDefaultAsync();
            var notification = new ApprovalNotification
            {
                StaffId = selfEvaluationRequest.CreatedBy,
                Message = $"{staffFullName} has submitted a self-evaluation on {submittedDateTime}.",
                IsActive = true,
                CreatedBy = selfEvaluationRequest.CreatedBy,
                CreatedUtc = DateTime.UtcNow
            };
            await _context.ApprovalNotifications.AddAsync(notification);
            await _context.SaveChangesAsync();
            selfEvaluation.ApprovalNotificationId = notification.Id;
            await _context.SaveChangesAsync();

            if (!string.IsNullOrEmpty(approver1.OfficialEmail))
            {
                await _emailService.SendSelfEvaluationNotificationToManagerAsync(approver1.OfficialEmail, managerName, staffFullName, submittedDateTime, approver1.Id);
            }
        ...
        public async Task<string> CreateManagerEvaluation(ManagerEvaluationRequest managerEvaluationRequest)
        {
            ...
            var managerName = $"{manager.FirstName}{(string.IsNullOrWhiteSpace(manager.LastName) ? "" : " " + manager.LastName)}";
            var staffFullName = $"{staff.FirstName}{(string.IsNullOrWhiteSpace(staff.LastName) ? "" : " " + staff.LastName)}";
            var reviewedOn = managerEvaluation.CreatedUtc.ToLocalTime().ToString("dd-MMM-yyyy 'at' HH:mm:ss");
            var notification = new ApprovalNotification
            {
                StaffId = staff.Id,
                Message = $"Your manager {managerName} has completed the evaluation for your KRA on {reviewedOn}.",
                IsActive = true,
                CreatedBy = managerEvaluationRequest.CreatedBy,
                CreatedUtc = DateTime.UtcNow
            };
            await _context.ApprovalNotifications.AddAsync(notification);
            await _context.SaveChangesAsync();
            managerEvaluation.ApprovalNotificationId = notification.Id;
            await _context.SaveChangesAsync();
                ...
                using (var stream = new MemoryStream())
                {
                    ...
                    using (var package = new ExcelPackage(stream))
                    {
                        ...
                        if (employeePerformanceReviews.Count > 0)
                        {
                            ...
                            if (!rowsToProcess.Any())
                            {
                                throw new ConflictException($"All selected staff have already been uploaded");
                            }
                            var newRecords = employeePerformanceReviews
                                .Where(x => !alreadyUploaded.Contains(x.EmpId))
                                .ToList();
        ...
        public async Task<List<HrUploadResponse>> GetHrUploadedSheet(int appraisalId, int year, string? quarter, int? month)
        {
            var response = await (from hr in _context.NonProductionEmployeePerformanceReviews
                                  ...
                                  where hr.Year == year && (quarter == null || hr.Quarter == quarter) && (month == null || hr.Month == month) && ap.IsActive && hr.AppraisalId == appraisalId
