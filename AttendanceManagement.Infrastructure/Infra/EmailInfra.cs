...
namespace AttendanceManagement.Infrastructure.Infra
{
    public class EmailInfra : IEmailInfra
    {
        ...
        public async Task SendSelfEvaluationNotificationToManagerAsync(string managerEmail, string managerName, string employeeName, string submittedDateTime, int approvedBy)
        {
            var subject = $"Self-Evaluation Submitted by {employeeName}";
            var body = $@"
            <p>Dear {managerName},</p>
            <p><strong>{employeeName}</strong> has submitted a self-evaluation for their KRA as part of the appraisal process.</p>
            <p><strong>Submitted On:</strong> {submittedDateTime}</p>
            <p>Please review the self-evaluation and proceed with your feedback.</p>
            <p>Best regards,
            <br/>HR Team</p>";

            await SendApprovalEmail(managerEmail, subject, body, approvedBy);
        }
    }
}