...
public class GradeMasterInfra : IGradeMasterInfra
{
    ...
    private const string GradeCacheKey = "AllGrades";
    public GradeMasterInfra(AttendanceManagementSystemContext context, IMemoryCache cache)
    {
        _context = context;
        _cache = cache;
    }
    ...
    {
        if (_cache.TryGetValue(GradeCacheKey, out var cachedObj) && cachedObj is List<GradeMasterResponse> cachedGrades)
        {
            return cachedGrades;
        }
        ...
        _cache.Set(GradeCacheKey, allGrade);
    ...
    public async Task<string> CreateGrade(GradeMasterRequest gradeMasterRequest)
    {
        ...
        var isDuplicate = await _context.GradeMasters
            .AnyAsync(g => g.Name.ToLower() == gradeMasterRequest.FullName.ToLower());
    ...
    public async Task<string> UpdateGrade(UpdateGradeMaster gradeMaster)
    {
        ...
        var existingGrade = await _context.GradeMasters
            .FirstOrDefaultAsync(g => g.Id == gradeMaster.GradeMasterId);
        ...
        {
            var isDuplicate = await _context.GradeMasters
                .AnyAsync(g => g.Id != gradeMaster.GradeMasterId &&
                               g.Name.ToLower() == gradeMaster.FullName.ToLower());
            if (isDuplicate) throw new ConflictException("Grade name already exists");
        }
