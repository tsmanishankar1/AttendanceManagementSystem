...
{
    public class LetterGenerationInfra : ILetterGenerationInfra
    {
        private readonly string _workspacePath;
        private readonly string _workspaceLogoAndSealPath;
        public LetterGenerationInfra(IWebHostEnvironment env)
        {
        ...
        public string GenerateConfirmationPdf(int staffCreationId, string designation, string title, string startDate, string endDate, string fileName, string staffName, string employeeCode)
        {
            ...
            string logoPath = Path.Combine(_workspaceLogoAndSealPath, "Logo.png");
            string footerPath = Path.Combine(_workspaceLogoAndSealPath, "Bottom Address Details.png");
            string watermarkPath = Path.Combine(_workspaceLogoAndSealPath, "Water Mark.png");
            string signatureImagePath = Path.Combine(_workspaceLogoAndSealPath, "Nirmala Digital signature.png");

            using (var fs = new FileStream(filePath, FileMode.Create, FileAccess.Write, FileShare.None))
            using (var pdfDoc = new Document(PageSize.A4, 70f, 70f, 54f, 36f))
            {
                var writer = PdfWriter.GetInstance(pdfDoc, fs);
                writer.PageEvent = new PdfPageEvents(footerPath, watermarkPath);
                pdfDoc.Open();
                ...
                logoImg.ScaleToFit(85f, 70f); // Adjust size as needed
                ...
                pdfDoc.Add(new Paragraph(" ") { SpacingAfter = 10f });
                ...
                iTextSharp.text.Image signatureImg = iTextSharp.text.Image.GetInstance(signatureImagePath);
                signatureImg.ScaleToFit(60f, 60f);
                signatureImg.Alignment = Element.ALIGN_LEFT;
                pdfDoc.Add(signatureImg);
                ...
                hrSignature.Add(new Chunk("Manager - HR", bodyFont));
                pdfDoc.Add(hrSignature);
...
*/
                ...
                void AddRow(string label, decimal currPA, decimal currPM, decimal revPA, decimal revPM, bool isBold = false, bool isSectionHeader = false, bool isFirstRow = false)
                {
                    ...
                    if (isSectionHeader)
                    {
                        var headerCell = new PdfPCell(new Phrase(label, font))
                        {
                        ...
                        var endCell = new PdfPCell(new Phrase(""))
                        {
                ...
                iTextSharp.text.Image signatureImg1 = iTextSharp.text.Image.GetInstance(signatureImagePath);
                signatureImg1.ScaleToFit(60f, 60f);
                signatureImg1.Alignment = Element.ALIGN_LEFT;
                doc.Add(signatureImg1);
