    ...
    public class StaffTransactionController : ControllerBase
    {
        ...
        public async Task<IActionResult> Create(ListAcademicDetailRequest academicDetailRequests)
        {
            ...
            catch (Exception ex)
            {
                await _loggingService.LogError(
                    "Academic Detail", "POST", "/api/StaffTransaction/CreateAcademicDetails",
                    ex.Message, ex.StackTrace ?? string.Empty,
                    ex.InnerException?.ToString() ?? string.Empty,
                    academicDetailRequests.CreatedBy,  
                    JsonSerializer.Serialize(academicDetailRequests)
                );
                return ErrorClass.ErrorResponse(ex.Message);
            }
        }
        ...
        public async Task<IActionResult> Update(ListAcademicDetailUpdateRequest academicDetailsRequests)
        {
            try
            {
                ...
                await _loggingService.AuditLog(
                    "Academic Detail", "POST", "/api/AcademicDetail/UpdateAcademicDetails",
                    updatedMessage,
                    academicDetailsRequests.UpdatedBy,  
                    JsonSerializer.Serialize(academicDetailsRequests)
                );
                return Ok(response);
            }
            catch (MessageNotFoundException ex)
        ...
        public async Task<IActionResult> Update(ListCertificationCourseUpdateRequest certificationCourses)
        {
            try
            {
                ...
                await _loggingService.AuditLog(
                    "Certification Detail", "POST", "/api/StaffTransaction/UpdateCertificationDetails", updatedMessage,certificationCourses.UpdatedBy, JsonSerializer.Serialize(certificationCourses)
                );
            ...
            {
                await _loggingService.LogError(
                    "Certification Detail", "POST", "/api/StaffTransaction/UpdateCertificationDetails",  ex.Message, ex.StackTrace ?? string.Empty,  ex.InnerException?.ToString() ?? string.Empty,  certificationCourses.UpdatedBy,    JsonSerializer.Serialize(certificationCourses)
                );
     ...
     [HttpGet("GetWorkHistorybyStaffId")]
        ...
        public async Task<IActionResult> Update(ListPreviousEmploymentUpdateRequest previousEmploymentUpdateRequest)
        {
            try
            {
                ...
                await _loggingService.AuditLog(
                    "WorkHistory Detail", "POST", "/api/WorkHistory/UpdateWorkHistory", message,previousEmploymentUpdateRequest.UpdatedBy,JsonSerializer.Serialize(previousEmploymentUpdateRequest)
                );
